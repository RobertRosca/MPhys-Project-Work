function ObsGenerateXML(ObsIDs, XML_out_dir="I:/FileZilla.xml")
    ftp_init()
    ...
    for ObsID in ObsIDs
        info("Generating file list for $(ObsID)")
        ...
        info("Found $(length(download_list)) files")
...
function ObsGenerateXMLBatch(local_archive="I:/.nustar_archive", log_file="I:/.nustar_archive/00000000000 - utility/download_log.csv", batch_size=100)
    ...
    for i = 0:obs_count-1
        if get(observations[obs_count-i, :Downloaded]) == 0
            append!(queue, [get(observations[obs_count-i, :ObsID])])
            print(get(observations[obs_count-i, :ObsID]))
            bs += 1
        end
